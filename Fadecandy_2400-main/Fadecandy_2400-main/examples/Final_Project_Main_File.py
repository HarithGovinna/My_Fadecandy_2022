...
#============================ Snake =========================================#
    ...
    def Snake(self):                             #Defining snakes self function 
        ...
        while led < 30:                           #While number of LEDs less than 30                                     
            for led in range (352):               #Run the python in the LED range of 352 as the length of the snake is 8.
                leds = [(0,0,0)]*360              #Colour of LED frame (white) 
                leds[led] = (255,140,0)           #Adding one LED to the front for the forward motion. Colour yellow 
                leds[led+1] = (255,140,0)           
                leds[led+2] = (0,255,0)           #Green colour
                leds[led+3] = (0,255,0)
                leds[led+4] = (0,255,0)
                leds[led+5] = (0,255,0)
            ...
            for led in range (352):               #Run the python in the LED range of 352
                leds = [(0,0,0)]*360              #Colour of LED frame (white) 
                leds[352- led] = (255,140,0)      #Adding one LED to the front for the forward motion. Colour Yellow 
                leds[352-led+1] = (255,140,0)           
                leds[352-led+2] = (0,255,0)       #Green colour
                leds[352-led+3] = (0,255,0)
                leds[352-led+4] = (0,255,0)
                leds[352-led+5] = (0,255,0)
                leds[352-led+6] = (255,140,0)     #Adding one LED to the front for the forward motion. Colour Yellow
        ...
        while Start:                                 #While start the loop is terminating.
...
#================================ Flag ==========================================#
    ...
    def Flag(self):                             #Defining Flag self function
        ...
        flag_positions1 = [[0,1,2,3,0,5,0,0,255,0.1], [3,4,5,6,0,5,255,255,0,0.1], [6,7,8,9,0,5,255,0,0,0.1], [9,10,11,12,0,5,255,255,255,0.1],
                          [12,13,14,15,0,5,255,165,0,0.1], [15,16,17,18,0,1,0,0,255,0.1], [15,16,17,18,1,2,255,255,0,0.1], [15,16,17,18,2,3,255,0,0,0.1], 
                          [15,16,17,18,3,4,255,255,255,0.1], [15,16,17,18,4,5,255,165,0,0.1]]     
        ...
        for i, item in enumerate(flag_positions1):                     #Implementing first tree list to the python's enumerate() to get a counter and the value from the iterable at the same time.
            for one in range(item[0], item[3]):                        #Range of the fisrt column using for loop
                ...
                client.put_pixels(leds1)
                sleep(item[9])                                         #Implement the sleep (Sleep by 0.1s) from the above array list using the item.
        ...
        flag_positions2 = [[57,58,59,60,0,5,0,0,255,0.1], [54,55,56,57,0,5,255,255,0,0.1], [51,52,53,54,0,5,255,0,0,0.1],
                          [48,49,50,51,0,5,255,255,255,0.1], [45,46,47,48,0,5,255,165,0,0.1], [42,43,44,45,0,1,0,0,255,0.1], [42,43,44,45,1,2,255,255,0,0.1],
                          [42,43,44,45,2,3,255,0,0,0.1], [42,43,44,45,3,4,255,255,255,0.1], [42,43,44,45,4,5,255,165,0,0.1]]         #Make a variable as heart to give the first line led position by an array list 
...
#============================== Star ========================================#
    ...
    def Blink(Self):                              #Defining Star self function
        ...
        for j in range(3):                         #Terminating 4 times 
    ...
    def Random_Dark (self):                   #Defining Dark self function
        ...
        while Turns<=2:                        #While the turns less than or equal to 2, the loop is terminating
            ...
            while led<60:                      #While number of LEDs less than 60, the loop is terminating else it stops.
                ...
                sleep(0.04)                                              #Sleep by 0.04 seconds
...
# ========================== Iterations=====================#
    ...
    def iterations(self):                                 #Defining Iterations self function
        ...
        while led<30:                                        #While number of LEDs less than 30, the loop is terminating else it stops.                                                               
            ...
            client.put_pixels(leds)                           #Draw a LED pixel in the frame.
            sleep(0.06)                                       #Sleep by 0.06 seconds
            led = led + 1                                     #Counting the number of turns in the while loop
...
#=============================== Covid 19 ====================================#
    ...
    def Covid(self):                                                  #Defining Covid-19 self function
        ...
        if (temp>37 and fever>3 and cough>3):                    #If temp is greater than 37, fever greater than 3 and dry cough greater than 3, the final printing statment is satisfying (You are positive with Coroana Virus). 
            ...
            led = 0
            led_frame1 = [(0,0,0)]*360  
            while led<60:                              #While number of LEDs less than 60, the loop is terminating else it stops.
                for rows in range(6):                  #Number of rows in the range of 6 using a for loop.
                    led_frame1[led + rows*60] = (255,0,0)       #Scroll from starting point to ending point
                sleep(0.03)                            #Sleep by 0.03 seconds
                client.put_pixels(led_frame1)          #Draw a LED pixel in the frame.
                led = led + 1                          #Counting the number of turns in the while loop                            
 
            led = 0
            while led<60:                                        #While number of LEDs less than 60, the loop is terminating else it stops.
                for rows in range(6):                            #Number of rows in the range of 6 using a for loop.                        
                    led_frame1[59-led + rows*60] = (0,0,0)       #Scroll from ending point to starting point.
                sleep(0.03)                                      
                client.put_pixels(led_frame1)
                led = led + 1          
        ...
        # ================= POISTIVE WORD =========================== #                   
            ...
            for sign in range (0,1):                   #The range of the row.
                for rows in range(0,6):                #The range of column.
                    frame[sign +rows*60] = (255,0,0)    #The colour of the LED bulbs.
                client.put_pixels(frame)                #Draw a LED pixel in the frame.
                sleep(0.03)                             #Sleep by 0.03 seconds.

            for sign in range (1,4):
                for rows in range(0,1):
                    frame[sign +rows*60] = (255,0,0)
            ...
            while led<60:                               #While number of LEDs less than 60, the loop is terminating else it stops.
                for rows in range(6):                   #Number of rows in the range of 6 using a for loop.               
                    led_frame1[led + rows*60] = (0,255,0)  #Scroll from starting point to ending point  
                sleep(0.03)                                #Sleep by 00.03 seconds
                client.put_pixels(led_frame1)              #Draw a LED pixel in the frame.
                led = led + 1                             #Counting the number of turns in the while loop                               
            ...
            for sign in range (0,1):                #The range of the row.
                for rows in range(0,6):             #The range of column.
                    frame[sign +rows*60] = (0,255,0)  #The colour of the LED bulbs.
                client.put_pixels(frame)              #Draw a LED pixel in the frame.
                sleep(0.03)                           #Sleep by 0.03 seconds.

            for sign in range (5,6):
                for rows in range(0,6):
                    frame[sign +rows*60] = (0,255,0)
                client.put_pixels(frame)
                sleep(0.03)
